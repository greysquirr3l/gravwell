...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    #[cfg(feature = "gpu")]
    {
        ...
        let runtime_result = pollster::block_on(async {
            run_demo().await
        });
...
async fn run_demo() -> gravwell::Result<()> {
    ...
    let gpu_calculator = match GpuDirectGravity::default().await {
        Ok(calc) => calc,
        Err(e) => {
    ...
    println!("Time per step: {:.2}ms", elapsed.as_millis() as f64 / num_steps as f64);
    println!("Simulated FPS: {:.1}", num_steps as f64 / elapsed.as_secs_f64());
    ...
    println!("GPU utilization: ~{} threads", 64 * ((particle_count + 63) / 64));
